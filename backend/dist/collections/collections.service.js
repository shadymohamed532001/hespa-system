    ...
    async reverse(id, dto, username) {
        return this.dataSource.transaction(async (manager) => {
            ...
            if (collection.status === CollectionStatus.DONE) {
                ...
                if (!account)
                    throw new NotFoundException('الحساب المرتبط غير موجود');
                if (account.commissionBalance < collection.commission) {
                    throw new BadRequestException('رصيد العمولة الحالي لا يكفي لعكس عمولة التحصيل');
                }
                account.balance += collection.amount;
