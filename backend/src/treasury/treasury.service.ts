...
export class TreasuryService {
  ...
  private async asset(manager: EntityManager, type: string, id?: string): Promise<TransferAsset> {
    if (type === 'treasury') {
  ...
  async transfer(dto: InternalTransferDto, username: string) {
    return this.dataSource.transaction('SERIALIZABLE', async (manager) => {
      ...
      if (source.key === target.key) throw new BadRequestException('المصدر والوجهة يجب أن يكونا مختلفين');
      if (source.balance < dto.amount) throw new BadRequestException('رصيد المصدر غير كافٍ');
